...
if [[ -n "$RISK_EVENTS" && ! -f "$RISK_EVENTS" ]]; then
  echo "REAL_RUNTIME_REJECTED: risk JSONL does not exist: $RISK_EVENTS" >&2
  exit 2
fi
[[ -d "$SDK_ROOT" ]] || {
  echo "REAL_RUNTIME_REJECTED: Unitree SDK2 Python source not found: $SDK_ROOT" >&2
  exit 2
}
...
if [[ -z "$NETWORK_INTERFACE" ]]; then
  ...
  if [[ ${#candidates[@]} -ne 1 ]]; then
    echo "REAL_RUNTIME_REJECTED: expected exactly one UP interface on 192.168.123.0/24; found ${#candidates[@]}" >&2
    ip -brief -4 addr >&2
    exit 2
  fi
...
ip -o -4 addr show dev "$NETWORK_INTERFACE" up \
  | awk '{print $4}' \
  | grep -q '^192\.168\.123\.' || {
    echo "REAL_RUNTIME_REJECTED: $NETWORK_INTERFACE is not UP on 192.168.123.0/24" >&2
    ip -brief -4 addr >&2
    exit 2
  }

ping -I "$NETWORK_INTERFACE" -c 1 -W 1 "$ROBOT_IP" >/dev/null || {
  echo "REAL_RUNTIME_REJECTED: Go2 is not reachable at $ROBOT_IP through $NETWORK_INTERFACE" >&2
  exit 2
}

exec 9>/tmp/go2_companion_motion_writer.lock
flock -n 9 || {
  echo "REAL_RUNTIME_REJECTED: another companion motion-writer launcher holds the lock" >&2
  exit 3
}
...
echo "Go2 real companion launcher"
echo "  robot:    $ROBOT_IP"
...
echo "  startup:   IDLE (START remains required)"

cd "$GATEWAY_ROOT"
python3 -c "import unitree_sdk2py" || {
  echo "REAL_RUNTIME_REJECTED: unitree_sdk2py import failed" >&2
  exit 2
}
exec python3 tools/go2_companion_console.py --execute
