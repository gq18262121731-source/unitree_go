...
def test_video_bridge_push_creates_alarm() -> None:
    ...
    original_websocket_manager = dependencies._websocket_manager

    class _FakeWebSocketManager:
        def __init__(self) -> None:
            self.alarms: list[dict[str, object]] = []
            self.alarm_queues: list[dict[str, object]] = []

        async def broadcast_alarm(self, payload: dict[str, object]) -> None:
            self.alarms.append(payload)

        async def broadcast_alarm_queue(self, payload: dict[str, object]) -> None:
            self.alarm_queues.append(payload)
    ...
    try:
        fake_websocket_manager = _FakeWebSocketManager()
        dependencies._robot_gateway_service = _FakeRobotGateway()
        dependencies._websocket_manager = fake_websocket_manager
        ...
        assert alarm.alarm_type == AlarmType.FALL_DETECTED
        ...
        assert alarm.metadata["robot_task"]["status"] == "blocked"
        assert alarm.metadata["robot_task"]["error"] == "COMPANION_RISK_LOCK_ACTIVE"
        assert len(fake_websocket_manager.alarms) == 1
        assert fake_websocket_manager.alarms[0]["id"] == alarm.id
        assert len(fake_websocket_manager.alarm_queues) == 1
        assert fake_websocket_manager.alarm_queues[0]["type"] == "alarm_queue"
        assert settings.fall_detection_target_elder_id == "elder_demo_01"
    finally:
        dependencies._robot_gateway_service = original_robot_gateway
        dependencies._websocket_manager = original_websocket_manager
