...
from backend.services.companion_risk_service import (
    ...
    DisabledCompanionMotionExecutor,
...
from backend.services.elder_companion_control_service import ElderCompanionControlService
...
_robot_companion_weather_provider: WeatherProvider | None = None
...
_companion_risk_service: CompanionRiskService | None = None
_elder_companion_control_service: ElderCompanionControlService | None = None
...
def _build_robot_companion_weather_provider() -> WeatherProvider:
...
def get_robot_companion_weather_provider() -> WeatherProvider:
    global _robot_companion_weather_provider
    if _robot_companion_weather_provider is None:
        _robot_companion_weather_provider = _build_robot_companion_weather_provider()
    return _robot_companion_weather_provider
...
def get_robot_companion_context_manager() -> RobotCompanionContextManager:
    ...
    if _robot_companion_context_manager is None:
        _robot_companion_context_manager = RobotCompanionContextManager(
            ...
            alarm_service=_alarm_service,
            analysis_service=_analysis_service,
            weather_provider=get_robot_companion_weather_provider(),
...
def get_robot_gateway_service() -> RobotGatewayService:
    ...
    desired_companion_base_url = str(
        _settings.companion_gateway_base_url or ""
    ).strip().rstrip("/")
    ...
    if (
        ...
        or _robot_gateway_service.companion_base_url != desired_companion_base_url
        ...
        _robot_gateway_service = RobotGatewayService(
            base_url=desired_base_url,
            companion_base_url=desired_companion_base_url,
...
def get_companion_risk_service() -> CompanionRiskService:
    global _companion_risk_service
    if _companion_risk_service is None:
        if _settings.companion_risk_executor_mode == "mock":
            executor = MockCompanionMotionExecutor()
        else:
            executor = DisabledCompanionMotionExecutor()
        _companion_risk_service = CompanionRiskService(executor)
    return _companion_risk_service


def get_elder_companion_control_service() -> ElderCompanionControlService:
    global _elder_companion_control_service
    if _elder_companion_control_service is None:
        _elder_companion_control_service = ElderCompanionControlService(
            settings=_settings,
            gateway=get_robot_gateway_service(),
            care_service=_care_service,
            risk_service=get_companion_risk_service(),
        )
    return _elder_companion_control_service
...
async def _ingest_video_bridge_alarm_event(event: dict[str, object]) -> AlarmRecord | None:
    ...
    risk_event = VisionFallEventAdapter().to_companion_risk_event(event)
    try:
        risk_transition = get_companion_risk_service().handle_event(risk_event)
        risk_transition_payload = risk_transition.model_dump(mode="json")
    except CompanionRiskConflict as exc:
        risk_transition_payload = {
            "accepted": False,
    ...
    enriched_metadata = {
        **metadata,
        ...
        "raw_event": event,
        "companion_risk": risk_transition_payload,
        "is_real_device": True,
    }
    ...
    robot_dispatch = {
        "ok": False,
        "status": "blocked",
        "error": "COMPANION_RISK_LOCK_ACTIVE",
        "reason": "Legacy fall-confirmation motion cannot acquire control while Companion risk lock is active.",
        "incident_id": risk_event.incident_id,
    }
    ...
    if not emitted:
        # A later Qwen transition for the same incident must still receive a
        # successful/idempotent response even when AlarmService suppresses a
        # second UI alarm inside its cooldown window. Risk state and Qwen
        # persistence have already been updated above.
        return next(
            (
                existing
                for existing in _alarm_service.list_alarms()
                if str(existing.metadata.get("incident_id") or "") == risk_event.incident_id
            ),
            None,
        )
