...
async def receive_video_bridge_fall_event(
    ...
    multimodal_review = metadata.get("multimodal_review") if isinstance(metadata.get("multimodal_review"), dict) else None
    incident_id = str(event.get("incident_id") or payload.incident_id or payload.event_id or "").strip()
    ...
    return VideoBridgeFallEventResponse(
        ...
        fall_prob=_clamp_probability(
            payload.fall_score
            if payload.fall_score is not None
            else payload.fall_prob
            if payload.fall_prob is not None
            else payload.confidence
        ),
        incident_id=incident_id,
        event_type=str(event.get("companion_event_type") or event.get("event_type") or payload.event_type),
        qwen_review_saved=bool(result.get("qwen_review_saved")),
