...
class VideoBridgeService:
    ...
    def __init__(
        ...
        *,
        ...
        self._data_dir = self._runtime_config_path.parent if runtime_config_path is not None else self._settings.data_dir
        self._alarm_ingest_callback = alarm_ingest_callback
        self._bridge_seen_keys: dict[str, float] = {}
        self._bridge_results: dict[str, AlarmRecord] = {}
        self._fall_event_adapter = VisionFallEventAdapter()
    ...
    async def receive_fall_event_async(
        ...
        *,
        ...
        qwen_review_saved = self._persist_qwen_review(event)
        result = await self._promote_event(
            event,
            source_label="vision_service_push",
            source_ip=source_ip,
        )
        result["qwen_review_saved"] = qwen_review_saved
        result["event"] = event
        return result
    ...
    def is_authorized_push(self, *, source_ip: str | None, push_token: str | None) -> bool:
        ...
        if bool(getattr(self._settings, "vision_bridge_production_mode", False)):
        ...
        *,
        ...
        if dedupe_key:
            ...
            if previous_seen_at is not None and now_monotonic - previous_seen_at < dedupe_window_seconds:
                ...
                existing_alarm = self._bridge_results.get(dedupe_key)
                return {
                    "promoted": False,
                    "suppressed": True,
                    "deduplicated": True,
                    "reason": "bridge_dedupe",
                    "dedupe_key": dedupe_key,
                    "alarm_id": existing_alarm.id if existing_alarm is not None else None,
                    "alarm_type": existing_alarm.alarm_type.value if existing_alarm is not None else None,
        ...
        if dedupe_key:
            ...
            if alarm is not None:
                self._bridge_results[dedupe_key] = alarm
    ...
    def _build_fall_event_from_push(self, payload: VideoBridgeFallEventRequest) -> dict[str, object]:
        ...
        probability = payload.fall_score if payload.fall_score is not None else payload.fall_prob
        if probability is None:
            probability = payload.confidence
        fall_prob = self._clamp_probability(probability)
        risk = payload.risk
        ...
        event["injury"] = dict(payload.injury) if isinstance(payload.injury, dict) else None
        ...
        return self._fall_event_adapter.normalize_event(event)
    ...
    def _promotion_event_from_latest(
        ...
        *,
        ...
        injury = dict(latest.get("injury") or {}) if isinstance(latest.get("injury"), dict) else None
        ...
        return self._fall_event_adapter.normalize_event(event), None
    ...
    def _default_runtime_config(self) -> dict[str, Any]:
        return {
            ...
            "push_token": str(self._settings.vision_service_push_token or "").strip(),
    ...
    def _persist_qwen_review(self, event: dict[str, object]) -> bool:
        metadata = event.get("metadata") if isinstance(event.get("metadata"), dict) else {}
        qwen_review = metadata.get("qwen_review") if isinstance(metadata.get("qwen_review"), dict) else None
        incident_id = str(event.get("incident_id") or "").strip()
        if not incident_id or qwen_review is None:
            return False
        safe_id = "".join(
            character if character.isalnum() or character in {"-", "_", "."} else "_"
            for character in incident_id
        )
        path = self._data_dir / "fall_events" / "qwen_review" / f"{safe_id}.json"
        try:
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text(json.dumps(qwen_review, ensure_ascii=False, indent=2), encoding="utf-8")
            return True
        except OSError as exc:
            LOGGER.warning("Failed to persist qwen review incident_id=%s: %s", incident_id, exc)
            return False
