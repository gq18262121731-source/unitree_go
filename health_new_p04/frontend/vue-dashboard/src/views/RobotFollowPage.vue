...
import CompanionLifecycleControl from "../components/robot/CompanionLifecycleControl.vue";
...
<template>
  <section class="page-stack robot-follow-page">
    <PageHeader
      ...
      title="机器人陪伴详情"
      description="社区端通过 health_new 启停 Companion Lifecycle；页面不会直连 8090，也不会发送原始 Move 指令。"
      :meta="pageMeta"
    >
      <template #actions>
    ...
    <article class="robot-follow-procedure">
      ...
      <ol>
        <li>
          <span>01</span>
          <div><strong>确认对象与绑定</strong><small>核对老人、Go2 名称和绑定状态，未绑定时禁止 START。</small></div>
        </li>
        <li>
          <span>02</span>
          <div><strong>检查空旷区域</strong><small>保持约 3×3m 空间，老人携带 UWB 遥控器，初始距离 1.5～2m。</small></div>
        </li>
        <li>
          <span>03</span>
          <div><strong>短距离启停</strong><small>慢走 2～3 步后停止，最终由 PC 点击 STOP 并确认回到 IDLE。</small></div>
        </li>
      </ol>
    </article>
