...
export type CompanionStartCheckState = "passed" | "failed" | "pending";

export interface CompanionStartCheck {
  key: string;
  label: string;
  state: CompanionStartCheckState;
  code?: string | null;
  detail?: string;
}

export interface ElderCompanionStatus {
  elder_id: string;
  elder_name: string;
  binding: {
    configured: boolean;
...
export async function requestJson<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      } else if (detailPayload && !Array.isArray(detailPayload)) {
        if (typeof detailPayload.code === "string" && detailPayload.code) {
          code = detailPayload.code;
        }
        if (typeof detailPayload.message === "string" && detailPayload.message) {
...
export const api = {
  ...
  getElderCompanionStatus: (elderId: string, token?: string, signal?: AbortSignal) =>
    requestJson<ElderCompanionStatus>(
      `${API_BASE}/elders/${encodeURIComponent(elderId)}/robot-companion/status`,
      { headers: withBearer(token), signal },
    ),
  startElderCompanion: (elderId: string, token?: string) =>
    requestJson<ElderCompanionStatus>(
      `${API_BASE}/elders/${encodeURIComponent(elderId)}/robot-companion/start`,
      { method: "POST", headers: jsonHeaders(token) },
    ),
  stopElderCompanion: (elderId: string, token?: string) =>
    requestJson<ElderCompanionStatus>(
      `${API_BASE}/elders/${encodeURIComponent(elderId)}/robot-companion/stop`,
      { method: "POST", headers: jsonHeaders(token) },
    ),
  listRobotTasks: (
