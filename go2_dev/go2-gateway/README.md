...
统一无线 Runtime 同时提供独立的视频边缘网关契约：`GET /healthz`、`GET /api/v1/video/status`、`GET /api/v1/robot/video` 和 `GET /stream.mjpg`。配置局域网 DNS 别名后，其他电脑通过 `http://robot-gateway:8093` 访问机器狗电脑，不保存其 DHCP 地址、不连接 Go2，也不引入 Unitree SDK 或 WebRTC 依赖。
