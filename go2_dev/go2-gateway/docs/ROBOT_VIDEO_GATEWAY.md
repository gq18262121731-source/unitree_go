...
> 当前网络：Go2 为 `192.168.8.254`，机器狗电脑 WLAN 为 `192.168.8.249`
...
```text
网关：http://192.168.8.249:8093
视频：http://192.168.8.249:8093/stream.mjpg
状态：http://192.168.8.249:8093/status
快照：http://192.168.8.249:8093/snapshot
```

Go2 的 `192.168.8.254` 只供机器狗电脑连接，主系统和移动端不要直接连接 Go2。

本回退版不使用 `robot-gateway.local` 或 mDNS 搜索，客户端暂时手动配置 `http://192.168.8.249:8093`。
