...
class Settings:
    ...
    mqtt_enabled: bool = _bool_env("GO2_MQTT_ENABLED", False)
    mqtt_broker_host: str = os.getenv("GO2_MQTT_BROKER_HOST", "127.0.0.1")
    mqtt_broker_port: int = _int_env("GO2_MQTT_BROKER_PORT", 1883)
    mqtt_client_id: str = os.getenv("GO2_MQTT_CLIENT_ID", "").strip()
    mqtt_topic_prefix: str = os.getenv("GO2_MQTT_TOPIC_PREFIX", "aiot")
    mqtt_keepalive_seconds: int = _int_env("GO2_MQTT_KEEPALIVE_SECONDS", 30)
    mqtt_qos: int = _int_env("GO2_MQTT_QOS", 1)
    mqtt_username: str = os.getenv("GO2_MQTT_USERNAME", "").strip()
    mqtt_password: str = os.getenv("GO2_MQTT_PASSWORD", "").strip()
