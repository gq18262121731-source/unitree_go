Create Codex pet row `running-right` for `momonga`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the existing soft white flying-squirrel identity, enormous pale-blue striped tail, pink cheeks, tiny bean limbs, and smug mischievous expression language.. Preserve silhouette, face, palette, material, proportions, markings, and props.
