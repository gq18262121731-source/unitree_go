...
import {
  ...
  robotFollowStateLabel,
  type RobotFollowTaskSnapshot,
} from "../utils/robotFollowPolicy";

const VIDEO_URL = GO2_VIDEO_BRIDGE.streamUrl;

const followTask = ref<RobotFollowTaskSnapshot>({ ...INITIAL_ROBOT_FOLLOW_TASK });
...
const canRecordFollowRequest = computed(() => followTask.value.state === "IDLE");
const canClearFollowRequest = computed(
  () => followTask.value.source === "local-intent" && followTask.value.state === "REQUESTED",
);
const followTaskLabel = computed(() => robotFollowStateLabel(followTask.value.state));
const followTaskSourceLabel = computed(() =>
  followTask.value.source === "robot" ? "机器人回执" : "本地意图",
);
...
function requestFollowTask() {
  if (!canRecordFollowRequest.value) return;
  followTask.value = recordLocalFollowRequest();
  appendAction("登记跟随请求", "请求未发送至机器人，不能解释为已接受或正在执行");
}

function cancelFollowRequest() {
  if (!canClearFollowRequest.value) return;
  followTask.value = clearLocalFollowRequest();
  appendAction("撤销跟随请求记录", "仅清除当前页面记录，未向机器人发送停止指令");
}
...
<template>
  <section class="page-stack robot-follow-page">
    <PageHeader
      ...
      title="机器狗自动跟随"
      description="通过 Go2 第一视角监看随行过程。当前后端未提供跟随任务接口，网页只登记本地跟随意图，不会提交任务或直接控制机器狗运动。"
      :meta="pageMeta"
    >
      <template #actions>
    ...
    <article class="robot-follow-procedure">
      ...
      <ol>
        ...
        <li>
          <span>03</span>
          <div><strong>登记操作意图</strong><small>页面记录不代表机器人确认；真实动作仍由工作人员通过随行遥控器执行。</small></div>
        </li>
      </ol>
    </article>
    ...
    <div class="robot-follow-layout">
      ...
      <aside class="robot-control-panel">
        <div class="robot-control-panel__identity">
          ...
          <span class="robot-control-panel__mode">无跟随任务接口</span>
        </div>

        <div class="robot-follow-task-state" role="status" aria-live="polite">
          <div>
            <span>跟随任务状态</span>
            <strong>{{ followTaskLabel }}</strong>
          </div>
          <span class="robot-follow-task-state__source">{{ followTaskSourceLabel }}</span>
          <p>{{ followTask.reason }}</p>
          <code>{{ followTask.state }}</code>
        </div>

        <div class="robot-control-actions" aria-label="跟随请求记录">
          <button
            type="button"
            class="robot-control-action robot-control-action--start"
            :disabled="!canRecordFollowRequest"
            @click="requestFollowTask"
          >
            <Play :size="19" fill="currentColor" />
            <span><strong>登记跟随请求</strong><small>仅记录意图，不发送运动指令</small></span>
          </button>
          <button
            type="button"
            class="robot-control-action robot-control-action--stop"
            :disabled="!canClearFollowRequest"
            @click="cancelFollowRequest"
          >
            <Square :size="18" fill="currentColor" />
            <span><strong>撤销请求记录</strong><small>不会向机器人发送停止指令</small></span>
          </button>
        </div>
    ...
    <section class="robot-follow-support">
      <article class="robot-follow-log">
        ...
        <div v-else class="robot-follow-log__empty">
          <Radio :size="24" />
          <p>当前页面还没有跟随请求记录。页面重新加载后，状态和记录都会清空并恢复为“未请求”。</p>
        </div>
      </article>
    </section>
  </section>
...
.robot-follow-task-state > div {
  display: grid;
  gap: 3px;
}

.robot-follow-task-state span,
.robot-follow-task-state p {
  color: #526b82;
  font-size: 0.75rem;
}

.robot-follow-task-state strong {
  color: #173f5f;
  font-size: 0.92rem;
}

.robot-follow-task-state__source {
  align-self: start;
  padding: 4px 7px;
  border-radius: 7px;
  background: #fff3dc;
  color: #8b5a0a !important;
  font-weight: 750;
}
