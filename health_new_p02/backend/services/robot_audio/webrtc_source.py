...
class Go2WebRTCAudioSource(AudioSource):
    ...
    def __init__(
        ...
        *,
        ...
        self._silence_rms_threshold = max(0, int(silence_rms_threshold))
        ...
        try:
            return await asyncio.wait_for(
                self._capture_segment(
                    max_duration_s=self._capture_duration_seconds,
    ...
    async def record_once(
        ...
        *,
        ...
        timeout = max(self._record_timeout_seconds, max_duration_s + 1.0)
        try:
            return await asyncio.wait_for(
                self._capture_segment(
    ...
    async def _capture_segment(
        ...
        *,
        ...
        async def receive_frame(frame: Any) -> None:
            nonlocal callback_error, captured_seconds, silent_seconds
            ...
            try:
                ...
                if captured_seconds >= max_duration_s or (
                    silence_timeout_s is not None
                    and silent_seconds >= silence_timeout_s
                ):
    ...
    @staticmethod
    def _pcm_rms(payload: bytes, sample_width: int) -> int:
        format_by_width = {1: "b", 2: "h", 4: "i"}
        typecode = format_by_width.get(sample_width)
        if typecode is None or len(payload) % sample_width:
            raise ValueError(f"unsupported PCM sample width: {sample_width}")
        samples = memoryview(payload).cast(typecode)
        if not samples:
            return 0
        mean_square = sum(int(sample) ** 2 for sample in samples) // len(samples)
