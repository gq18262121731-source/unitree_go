...
def test_go2_webrtc_source_finishes_after_silence_timeout() -> None:
    silent_frame = _FakeFrame(b"\x00\x00" * 960)
    connection = _FakeConnection([silent_frame, silent_frame, silent_frame])
    source = Go2WebRTCAudioSource(
        "192.168.123.161",
        connection_factory=lambda: connection,
    )
    ...
    capture = asyncio.run(
        source.record_once(max_duration_s=1.0, silence_timeout_s=0.02)
    )

    assert capture.data == silent_frame.to_ndarray().tobytes() * 2
    assert connection.audio.switch_calls == [True, False]
    assert connection.disconnected is True
