...
import RobotReadonlyTelemetryPanel from "../components/robot/RobotReadonlyTelemetryPanel.vue";
...
const isReadonlyMode = computed(() => telemetry.value?.provider === "unitree_readonly");

const pageMeta = computed(() => [
  "设备 Go2 EDU",
  isReadonlyMode.value ? "Provider Unitree Readonly" : "Provider Mock",
  "真实运动已禁用",
]);
...
<template>
  <section class="page-stack robot-status-page">
    ...
    <RobotReadonlyTelemetryPanel
      v-if="isReadonlyMode && telemetry"
      :telemetry="telemetry"
    />

    <div v-else class="robot-status-mock-banner" role="status">
      <Shield :size="21" />
      <div>
