...
export type PageKey = "overview" | "members" | "robot-tasks" | "robot-status" | "robot-navigation" | "robot-emergency" | "robot-follow" | "report" | "agent" | "family" | "debug" | "none";
...
export function useHashRouting(sessionUser: Ref<SessionUser | null>) {
  ...
  const allowedPages = computed<PageKey[]>(() => {
    ...
    if (sessionUser.value.role === "community" || sessionUser.value.role === "admin") {
      return ["overview", "members", "robot-tasks", "robot-status", "robot-navigation", "robot-follow", "robot-emergency", "report", "agent"];
    }
    return [];
  });
