...
export function useRobotStatus() {
  ...
  const telemetry = ref<RobotReadonlyTelemetryIntegration | null>(null);
  ...
  async function refresh(options: { initial?: boolean } = {}) {
    ...
    const results = await Promise.allSettled([
      robotNavigationApi.getRobotStatus(controller.signal)
        .then((value) => ({ kind: "legacy" as const, value })),
      robotNavigationApi.getRobotDiagnostics(controller.signal)
        .then((value) => ({ kind: "diagnostics" as const, value: value.data })),
      robotNavigationApi.getNavigationCapabilities(controller.signal)
        .then((value) => ({ kind: "capabilities" as const, value: value.data })),
      robotNavigationApi.getNavigationState(controller.signal)
        .then((value) => ({ kind: "navigation" as const, value: value.data })),
      robotNavigationApi.getReadonlyTelemetry(controller.signal)
        .then((value) => ({ kind: "telemetry" as const, value })),
    ]);

    if (disposed || controller.signal.aborted) return;
    const labels = ["机器人基础状态", "机器人诊断", "导航能力", "导航状态", "只读遥测"];
    results.forEach((result, index) => {
      if (result.status === "fulfilled") {
        const item = result.value;
        if (item.kind === "legacy") legacyStatus.value = item.value;
        if (item.kind === "diagnostics") diagnostics.value = item.value;
        if (item.kind === "capabilities") capabilities.value = item.value;
        if (item.kind === "navigation") navigationState.value = item.value;
        if (item.kind === "telemetry") telemetry.value = item.value;
        return;
      }
  ...
  return {
    ...
    hasData: computed(() => Boolean(
      diagnostics.value || navigationState.value || legacyStatus.value || telemetry.value,
    )),
  };
}
