...
export const api = {
  ...
  getRobotStatus: () => requestJson<RobotStatusResponse>(`${API_BASE}/robot/status`),
  listRobotTasks: (params: { status?: string; elder_id?: string; outcome?: string; limit?: number } = {}) => {
    const query = new URLSearchParams();
    if (params.status) query.set("status", params.status);
    if (params.elder_id) query.set("elder_id", params.elder_id);
    if (params.outcome) query.set("outcome", params.outcome);
    query.set("limit", String(params.limit ?? 100));
    return requestJson<RobotTaskListResponse>(`${API_BASE}/robot/tasks?${query.toString()}`);
  },
  getRobotTask: (taskId: string) => requestJson<RobotTaskDetailResponse>(`${API_BASE}/robot/tasks/${taskId}`),
  getRobotTaskTimeline: (taskId: string) =>
    requestJson<RobotTaskTimelineResponse>(`${API_BASE}/robot/tasks/${taskId}/timeline`),
  getRobotTaskObservation: (taskId: string) =>
    requestJson<RobotTaskObservationResponse>(`${API_BASE}/robot/tasks/${taskId}/observation`),
