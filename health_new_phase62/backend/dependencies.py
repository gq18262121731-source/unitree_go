...
_robot_readonly_telemetry_service: RobotReadonlyTelemetryService | None = None
...
def get_robot_readonly_telemetry_service() -> RobotReadonlyTelemetryService:
    global _robot_readonly_telemetry_service
    desired_mode = _settings.robot_telemetry_provider
    desired_path = str(_settings.robot_readonly_snapshot_path or "").strip()
    desired_url = str(_settings.robot_readonly_snapshot_url or "").strip()
    desired_timeout = max(0.1, float(_settings.robot_readonly_snapshot_timeout_seconds))
    if (
        _robot_readonly_telemetry_service is None
        or _robot_readonly_telemetry_service.integration_mode != desired_mode
        or _robot_readonly_telemetry_service.snapshot_path != desired_path
        or _robot_readonly_telemetry_service.snapshot_url != desired_url
        or abs(_robot_readonly_telemetry_service.timeout_seconds - desired_timeout) > 1e-9
    ):
        _robot_readonly_telemetry_service = RobotReadonlyTelemetryService(
            integration_mode=desired_mode,
            snapshot_path=desired_path,
            snapshot_url=desired_url,
            timeout_seconds=desired_timeout,
        )
    return _robot_readonly_telemetry_service
